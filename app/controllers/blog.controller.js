...
exports.blogAdd = async (req, res) => {
  try {
    ...
    if (req.file == undefined) {
      return res
        .status(400)
        .send({ status: false, message: "Please upload a file!" });
    }
    ...
    await sharp(req.file.buffer)
      .resize({ width: 418, height: 210 })
      .toFile(__basedir + "/uploads/blogs/listblogs/" + newFilename);
    await sharp(req.file.buffer)
      .resize({ width: 1254, height: 566 })
      .toFile(__basedir + "/uploads/blogs/detailblogs/" + newFilename);
    ...
    if (req.body.title.length > 200) {
      return res.status(400).send({
        success: false,
        message: "Title length should be less than 200 character!",
      });
    }
    ...
    if (existblog) {
      return res
        .status(400)
        .send({ success: false, message: "Blog already exist!" });
    }
...
exports.getAllBlogs = async (req, res) => {
  var fullUrl =
    ...
    "://" +
    req.get("host") +
    "/princetonhive/img/blogs/listblogs/";
  try {
...
exports.getActiveBlogs = async (req, res) => {
  var fullUrl =
    ...
    "://" +
    req.get("host") +
    "/princetonhive/img/blogs/detailblogs/";
  try {
    const allBlogs = await blogs.findAll({
      where: {
        status: "1",
...
exports.updateBlog = async (req, res) => {
  try {
    ...
    if (!(req.body.status == 0) && !(req.body.status == 1)) {
      return res
        .status(400)
        .send({ message: "Invalid input value for enum blog_status" });
    }
    if (!req.body.title.trim()) {
