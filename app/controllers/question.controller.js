...
exports.questionAdd = async (req, res) => {
  try {
    ...
    await sharp(req.file.buffer)
      ...
      .toFile(__basedir + "/uploads/question/" + newFilename);
    ...
    if (isNaN(req.body.departments)) {
      return res
        .status(400)
        .send({ message: "Please enter numeric value in department!" });
    }
    ...
    if (
      ...
      return res
        .status(400)
        .send({ message: "Invalid input value for enum level!" });
    }
    ...
    res
      .status(200)
      .send({ success: true, message: "Question added successfully!" });
  } catch (error) {
...
exports.updateQuestion = async (req, res) => {
  try {
    ...
    if (!(req.body.status == 0) && !(req.body.status == 1)) {
      return res
        .status(400)
        .send({ message: "Invalid input value for enum question_status" });
    }
    if (!req.body.questionName.trim()) {
      return res.status(400).send({ message: "Please enter Question name!" });
    }
